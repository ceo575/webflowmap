...
async function getSafeUserName(userId: string): Promise<string | null> {
  try {
...
async function getPracticeCards(userId: string): Promise<PracticeTopicCard[]> {
  // 1) Primary path: weakness-driven cards
  try {
    ...
    const cards = weaknesses
      .map((item: any) => {
        ...
        return {
          ...
          gradeLabel: latestExam?.grade ?? "--",
          mastery: Math.max(0, Math.min(100, Math.round(item.score))),
          status: getPracticeStatus(item.score),
          questionCount,
        } as PracticeTopicCard;
      })
      .filter((item: PracticeTopicCard) => item.questionCount > 0);
  ...
  // 2) Fallback path: build cards from published exams (no weakness dependency)
  try {
    const exams = await prisma.exam.findMany({
      where: { isPublic: true, topicId: { not: null } },
      select: {
        topicId: true,
    ...
    for (const exam of exams as any[]) {
      ...
      const current = byTopic.get(exam.topicId);
      if (!current) {
        ...
        current.questionCount += exam._count.questions;
      }
    }

    const cards = Array.from(byTopic.values());
    if (cards.length > 0) return cards;
  } catch {
    // continue to final fallback
  }

  // 3) Final fallback: render empty state instead of throwing
  return [];
}
...
export default async function PracticePage({
  ...
  const userName = await getSafeUserName(userId);
  const cards = await getPracticeCards(userId);
  ...
  const filteredCards =
    selectedSubject === "Tất cả"
      ? cards
      : cards.filter((item: PracticeTopicCard) => item.subject.toLowerCase() === selectedSubject.toLowerCase());
    ...
    <div className="mx-auto w-full max-w-5xl space-y-6 pb-10">
      ...
      {filteredCards.length === 0 && (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">
          Hiện chưa có dữ liệu chủ đề để luyện tập. Vui lòng thử lại sau.
        </div>
      )}
      ...
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-8 py-10 text-center">
        ...
        <p className="mx-auto mt-1 max-w-xl text-sm text-slate-500">
          {userName ? `${userName}, ` : ""}hoàn thành các bài luyện tập trên để cải thiện điểm số trung bình của bạn lên trên 8.0.
        </p>
      </div>
    </div>
  );
