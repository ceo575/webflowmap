...
export async function GET(request: Request) {
  ...
  const topics = weaknesses
    .map((entry: any) => {
      const latestExam = entry.topic.exams.find((exam: any) => Boolean(exam.subject)) ?? entry.topic.exams[0];
      ...
      return {
        ...
        questionCount: entry.topic.exams.reduce((sum: number, exam: any) => sum + exam._count.questions, 0),
      };
    })
    .filter((topic: any) => topic.questionCount > 0)
    .filter((topic: any) => (subject ? topic.subject.toLowerCase() === subject.toLowerCase() : true));
